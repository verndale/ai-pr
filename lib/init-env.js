...
/**
 * Build text to append so ai-pr env placeholders / docs exist. Returns null if nothing to add.
 * Keys listed in `extraAssignedKeys` count as already satisfied.
 * @param {string} existing
 * @param {Set<string> | undefined} [extraAssignedKeys]
 * @returns {string | null}
 */
function buildAiPrEnvAppend(existing, extraAssignedKeys) {
 ...
 * @param {string} bundledPath Path to shipped `.env-example` (full template on disk).
...
function mergeAiPrEnvFile(destPath, bundledPath, options = {}) {
  ...
  const append = buildAiPrEnvAppend(text, extraAssignedKeys);
  if (append !== null) {
