...
function injectAiPrDocsForExistingKeys(content) {
  ...
  if (!hasOurDocForKey(lines, "GH_TOKEN")) {
    const idx = lines.findIndex(
      (line) => /^\s*GH_TOKEN\s*=/.test(line) || /^\s*GITHUB_TOKEN\s*=/.test(line),
    );
    if (idx !== -1) {
  ...
  tryInject(/^\s*PR_BASE_BRANCH\s*=/, DOC_PR_BASE, "PR_BASE_BRANCH");
  tryInject(/^\s*PR_DRAFT\s*=/, DOC_PR_DRAFT, "PR_DRAFT");
  tryInject(/^\s*PR_AI\s*=/, DOC_PR_AI, "PR_AI");
  tryInject(/^\s*PR_AI_ENDPOINT\s*=/, DOC_PR_AI_ENDPOINT, "PR_AI_ENDPOINT");
  tryInject(/^\s*PR_AI_API_KEY\s*=/, DOC_PR_AI_API_KEY, "PR_AI_API_KEY");
  tryInject(/^\s*PR_AI_MODEL\s*=/, DOC_PR_AI_MODEL, "PR_AI_MODEL");
...
function buildAiPrEnvAppend(existing) {
  ...
  if (!keys.has("GH_TOKEN") && !keys.has("GITHUB_TOKEN")) {
    parts.push(`${DOC_GH_TOKEN[0]}\nGH_TOKEN=\n`);
  }
 ...
 */
function mergeAiPrEnvFile(destPath, bundledPath, options = {}) {
