...
async function generateAiSummary({ title, commits, fileChanges, allowedHashes }) {
  ...
  const model = process.env.PR_AI_MODEL || "default";
...
async function generateAiLabelsAndChecklist({ title, body, commits }) {
  ...
  const payload = { model, instructions: system, input: user, temperature: 0 };
  ...
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`AI labels/checklist endpoint failed: ${res.status}\n${text}`);
  }
